...
def prepare_data_pipelines(
    ...
    """Динамічне об'єднання пакетів."""
    processed_dir = Path(config["data"]["processed_dir"])
    # Перевіряємо, чи вже є готові файли
    train_csv = processed_dir / "train_registry.csv"
    val_csv = processed_dir / "val_registry.csv"
    test_csv = processed_dir / "test_registry.csv"

    if train_csv.exists() and val_csv.exists() and test_csv.exists():
        logger.info(">>> Processed data found. Loading from CSV...")
    ...
    # Зберігаємо CSV для наступних запусків
